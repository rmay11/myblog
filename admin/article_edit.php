...
if($sub){
	...
	$catid=$_POST['colId'];
	$c_time=time();
	$conn->query("update article set title='$title',author='$author',content='$content',keyword='$keyword',catid='$catid',c_time='$c_time' where id='$id'");
	if($conn->affected_rows){
		redirect('2','article_edit.php','修改成功');
	}else{
                        ...
                        <tbody><tr>
                            ...
                            <td>
                                <select name="colId" id="catid" class="required">
                                	<?php 
                                		$result_1=$conn->query(" select * from cate where id = '$t_id' ");
                                		$result_2=$conn->query("select * from cate where id != '$t_id'");
                                		
                                		$row_1=$result_1->fetch_assoc();
                                	?>
                                    <option ><?php echo $row_1['class_name']; ?></option>
                                    <?php 
                                    while($row_2=$result_2->fetch_assoc()){
                                    	
                                    	
                                    ?>
                                    <option ><?php echo $row_2['class_name']; ?></option>
                                    <?php	
                                    }	
                            ...
                            <tr>
                                ...
                                <td>
                                    <input class="common-text required" id="title" name="title" value="<?php echo $row['title']; ?>" size="50" value="" type="text">
                                </td>
                            </tr>
                            <tr>
                                <th>作者：</th>
                            ...
                            <tr>
                                <th>关键字</th>
                                <td><input class="common-text" name="keyword" size="50" value="<?php echo $row['keyword']; ?>" type="text" placeholder="输入关键字以空格或逗号隔开"></td>
                            </tr>
                            <tr>
                                <th>内容：</th>
